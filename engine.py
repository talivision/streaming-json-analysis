...
1. Structural Fingerprinting
   ...
   value types) without looking at actual values. Two objects with the
   same structure get the same fingerprint.
...
#   Same shape → same fingerprint → same type.
...
def structural_fingerprint(obj: dict) -> str:
    ...
    Returns a 12-character hex string. Objects with identical structure
    (same keys, nesting, value types) produce the same fingerprint.
    Actual values are ignored — only the shape matters.
    """
    shape = extract_shape(obj)
    # json.dumps with sort_keys gives a stable, deterministic string
    shape_str = json.dumps(shape, sort_keys=True)
    return hashlib.sha256(shape_str.encode()).hexdigest()[:12]
...
def extract_shape(value: Any) -> Any:
    ...
    Lists become [shape(first_element)] — first element is representative.
    ...
    if value is None:
        ...
        if not value:
            return ["empty"]
        # Use the first element's shape as representative of the whole list.
        # This works well for homogeneous arrays (the common case).
...
def shape_similarity(shape_a: Any, shape_b: Any) -> float:
    ...
    For dicts: Jaccard similarity on key sets (|intersection| / |union|).
    For other types: 1.0 if identical, 0.0 if different.
    ...
    def register(self, obj: dict, timestamp: float) -> tuple[str, bool]:
        ...
        if is_new:
            ...
            for existing_id, existing_type in self.types.items():
                sim = shape_similarity(obj_shape, existing_type.shape)
...
class CorrelationEngine:
    ...
    def __init__(self, baseline: BaselineModel):
        ...
        # Observations per period: period_id → [(type_id, latency), ...]
        self._period_observations: dict[int, list[tuple[str, float]]] = defaultdict(list)
    ...
    def observe(self, type_id: str, timestamp: float):
        """
        ...
        if current is not None:
            ...
            if latency >= 0:
                self._period_observations[current.id].append((type_id, latency))
    ...
    def correlations(self, action_label: str) -> list[dict]:
        ...
        for period in closed:
            for type_id, latency in self._period_observations.get(period.id, []):
