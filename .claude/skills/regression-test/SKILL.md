...
| G2 | Append growth is **not** treated as rotation | While analyzer is running, write a new event to `/tmp/json_demo/stream.jsonl` (or rely on http_stream.py's built-in writer). | UI keeps ingesting smoothly; no error; analyzer does not exit. Regression: whole-file ETag mismatch on every append, or an overly-aggressive shrink check, would cause a bail. |
...
**G4 catches `mark_dirty` short-circuits.** If `source_exists()` returns false for HTTP on transient 404/416, saves get skipped silently. The fix is to make HTTP `source_exists()` always return true — annotation persistence does not depend on whether the remote endpoint is reachable right this instant.
